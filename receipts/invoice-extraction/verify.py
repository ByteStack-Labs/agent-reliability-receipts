...
isolate the root cause. Run it from this directory:
...
fixtures/invoice-extraction/data/*.jsonl; nothing is read back from
results/*.json. If a number cannot be reproduced, the script exits non-zero.
"""
...
def find_fixture(start):
    """Walk up from this file to the repo root and locate the committed fixture.
    Rename-proof: independent of where the receipt is placed, as long as it lives
    inside the same repo as fixtures/invoice-extraction/."""
    rel = os.path.join("fixtures", "invoice-extraction", "data", "eval.jsonl")
    d = start
    while True:
        if os.path.isfile(os.path.join(d, rel)):
            return os.path.join(d, "fixtures", "invoice-extraction")
        parent = os.path.dirname(d)
        if parent == d:
            print("verify: could not locate fixtures/invoice-extraction", file=sys.stderr)
            sys.exit(2)
        d = parent
...
# How the fixture builds eval-format text (mirrors generate.fmt_eval). Re-declared
# here so the ablations are self-contained and depend on no private helper.
LABELS = {
    "invoice_id": "Invoice",
    "vendor": "Vendor",
    "client": "Client",
...
def eval_format(truth, order=None):
    return "\n".join(f"{LABELS[f]}: {truth[f]}" for f in (order or FIELDS))
...
def main():
    ...
    for split in ("eval", "production"):
        ...
        print(f"  {split:11s} n={len(recs)}  exact_match={em_rate}  mean_field_accuracy={fa_mean}")
    ...
    ok &= check("aggregate drop", drop, 0.1375)
    ...
    for r in prod:
        ...
        if not em:
            wrong += 1
            # "silent" = all 11 fields populated, non-empty: indistinguishable
            # from a correct record. "detectable" = at least one empty field,
            # which a downstream null-check would flag.
    ...
    print(f"  SILENT (well-formed, no signal): {silent}  by shift: {dict(sorted(silent_by.items()))}")
    print(f"  detectable (empty field present): {detectable}  by shift: {dict(sorted(detectable_by.items()))}")
    ...
    ok &= check("wrong (not-exact) count", wrong, 55)
    ...
    banner("4. ABLATION A  (hold content + delimiter + line-count constant; move 2 fields)")
    truth = load("eval")[0]["truth"]
    base_pred = extract(eval_format(truth))
    ...
    swap_pred = extract(eval_format(truth, swapped))
    sfa, sem = score(swap_pred, truth)
    changed = [f for f in FIELDS if swap_pred[f] != truth[f]]
    print(f"  one adjacent swap (vendor<->client): exact_match={sem} field_accuracy={sfa:.4f}")
    print(f"  fields now wrong: {changed}")
    print(f"  extractor put '{swap_pred['vendor']}' in vendor (truth '{truth['vendor']}'),")
    print(f"               '{swap_pred['client']}' in client (truth '{truth['client']}')")
    ...
    print(f"  fields wrong: {len(vwrong)}/{len(FIELDS)} (a single line shifts every position)")
